...
/* ---- layout ---- */
/* minmax(0,...) on every track, and min-width:0 on the children. A bare `1fr`
   floors at the widest child's min-content, so one long unbreakable line in a
   <pre> or a table blows the column past the container - which it did on mobile,
   clipping ~127px of every page that has a contents rail. */
.mt-layout{display:grid;grid-template-columns:240px minmax(0,1fr);gap:3.5rem;margin-top:3rem}
.mt-layout>*{min-width:0}
@media(max-width:940px){.mt-layout{grid-template-columns:minmax(0,1fr);gap:0}}
